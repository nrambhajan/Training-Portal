...
import {
  ...
  List, FileText, Lightbulb, RefreshCw
} from "lucide-react";

function ShortAnswerCard({ question, attempt, onSubmit }) {
...
function MCQCard({ question, attempt, onSubmit }) {
  ...
  const locked = attempt?.is_correct;
...
function OutputCard({ question, attempt, onSubmit }) {
  const [text, setText] = useState("");
  const [loading, setLoading] = useState(false);
...
export default function TraineeLab() {
  ...
  async function handleVerify(questionId) {
    try {
      ...
      else toast.error("Verification failed. Check the output and try again.");
      load();
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="flex flex-col gap-6">
          {items.map(({ question: q, attempt }, idx) => (
            <div key={q.id} className={`bg-white rounded-xl border-2 transition ${attempt?.is_correct ? "border-green-200" : attempt ? "border-red-100" : "border-gray-200"}`}>
              ...
              <div className="px-6 py-5">
                ...
                {q.type === "mcq" && (
                  <MCQCard question={q} attempt={attempt} onSubmit={handleSubmit} />
                )}
                {q.type === "practical" && (
                  <PracticalCard question={q} attempt={attempt} onVerify={handleVerify} />
                )}
                {q.type === "output" && (
                  <OutputCard question={q} attempt={attempt} onSubmit={handleSubmit} />
                )}
                {q.type === "short_answer" && (
                  <ShortAnswerCard question={q} attempt={attempt} onSubmit={handleSubmit} />
                )}
